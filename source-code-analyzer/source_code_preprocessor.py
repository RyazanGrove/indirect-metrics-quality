...
from shared.constants import verification_start_date, vpr_start_date, services_metadata
...
for service_index, current_service in enumerate(services_metadata["source_code"]):
    ...
    source_code_data.to_excel('..output\\source_code\\source_code_for_ml' + service_abr + '.xlsx')
    ...
    print('\nNumber of items in the dataset for service {0}: {1}'.format(service_abr, source_code_data.shape[0]))
